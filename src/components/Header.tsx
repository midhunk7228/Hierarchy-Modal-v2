...
const DEFAULT_DASHBOARD: DashboardLayout = {
  ...
  logos: ["/Nike.png", "/McDonalds.png","/Starbucks.png","/pepsi.png","/Intel.png","/coca-cola.png","/Netflix.png","/pg.png","/Mastercard.png","/NVIDIA.png","/Samsung.png","/Nestle.png","/Louis.png","/Meta.png","/Amazon.png","/Chevron.png","/Tesla.png"],
  grid: {
    columns: 12,
    rows: 8,
    gap: 16,
          ...
          <div className="hidden xl:flex lg:flex md:flex items-center gap-2 sm:gap-3">
            {currentDashboard?.logos?.slice(6).map((logo, index) => (
              <img
                ...
                className={`h-5 sm:h-6 md:h-7 cursor-pointer transition-all flex-shrink-0 ${
                  selectedLogoIndex === index + 6 ? "" : "grayscale hover:grayscale-0"
                }`}
              />
            ))}
          </div>
